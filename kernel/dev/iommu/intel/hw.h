...
struct RootEntry {
    ...
    void ReadFrom(volatile RootEntry* dst) {
        raw[0] = dst->raw[0];
        raw[1] = dst->raw[1];
    }
    void WriteTo(volatile RootEntry* dst) {
        dst->raw[0] = raw[0];
        dst->raw[1] = raw[1];
    }
};
...
struct RootTable {
    ...
    volatile RootEntry entry[kNumEntries];
};
...
struct ContextTable {
    static constexpr size_t kNumEntries = 256;
    volatile ContextEntry entry[kNumEntries];
};
...
struct ExtendedContextTable {
    ...
    volatile ExtendedContextEntry entry[kNumEntries];
};
